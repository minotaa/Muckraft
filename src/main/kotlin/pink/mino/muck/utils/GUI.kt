...
class GuiBuilder : Listener {
    ...
    fun onInventoryClick(e: InventoryClickEvent) {
        if (e.currentItem != null) {
            if (e.currentItem!!.type != Material.AIR) {
                if (ChatColor.stripColor(e.view.title)
    ...
    fun onPlayerClose(event: InventoryCloseEvent) {
        if (ChatColor.stripColor(event.view.title)
                .equals(ChatColor.stripColor(name), ignoreCase = true)
        ) {
    ...
    init {
        Bukkit.getPluginManager().registerEvents(this, JavaPlugin.getPlugin(Main::class.java))
        name = "Inventory"
        rows = 1
        items = HashMap()
